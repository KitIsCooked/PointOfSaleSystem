<?php
    ...
    if(isset($_POST['btnsave'])) {
        ...
        $proceed = true; // Flag to control if we should proceed with insertion
        ...
        if(!empty($barcode)) {
            ...
            if($select->rowCount() > 0) {
                $_SESSION['status'] = "Barcode already exists. Please use a different barcode.";
                $_SESSION['status_code'] = "error";
                $proceed = false; // Don't proceed with product insertion
            }
        }

        // Only proceed if no duplicate barcode was found
        if($proceed) {
